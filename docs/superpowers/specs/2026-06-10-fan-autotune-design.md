...
4. **增量拟合跳点**(实现修订):**实测 ≥ 6 点后**才允许跳点(4 参数拟合要数据胜过要分钟),
   每点重拟合一次,预测 ≥ 89 °C 的后续点跳过(标记 skipped)。跳点预测用**未加保守偏置**
   的模型 —— 偏置是曲线安全裕度,拿来剪枝会过度跳点掏空拟合数据。
...
- CPU 中止线:`T_abort = 91 °C`(**失控线**,实现修订)。真实硅片在 TjMax(9950X3D 为 89)
  会瞬时降频自钳,"过热"的网格点会稳定在 89.x °C、功耗下降 —— 这是**完全有效的
  (P, w, T) 降频钳制样本**,照常采集而不是中止;91 只在硅片保护失效/传感器谎报的真失控
  时触发。**与用户目标解耦**:激进目标 + 弱散热器时测量照样完成并产出「散热器不足」的
  诚实警告。验证阶段线为 `min(effectiveTarget + 6, 91)`,且**入场先全速冷却**到
  目标 −2 °C(≤90 s)再交给曲线驱动 —— 网格刚结束时die 还热、风扇还低,直接开验证会
  误触线。触发即:全部风扇 100% → 杀全部负载 → 恢复原配置 → `Aborted("温度失控")`。
...
4. **比例带**:`B = 8 °C`,`T_low = effectiveTarget − B`。
   ...
   - **平坦分支判据(实地修订)**:`T(P_design, floor) ≤ effectiveTarget`(锚定目标本身,而非
     t_low)→ 曲线 = 底线平线到目标点(锚 `w_req`)+ 安全爬升。强散热器/低功耗机器的正确
     输出就是平坦安静,而不是穿过日常温区的无谓爬坡。
   - **带内点钳制**:每个调度点的 w 按分量 `min(w, w_req)` — 带内任何点不得比满载锚点更响,
     否则单调性会把整条曲线拖到一个病态早期点上(实地"60°C 悬崖"bug)。包络余量 0.3→1.0。
   - **退化拟合警告**:风扇权威(`T(P,floor)−T(P,full)`)< 3°C → `fansBarelyMatter` 警告,
     解释曲线为何接近平坦(数据的诚实结论);GPU 侧 < 2°C → `caseBarelyHelpsGpu`,辅助曲线
     保持平坦(机箱风量帮不上忙时,超温区冲上限是纯噪音)。
5. **完整组级 w 曲线**(每组 9 点):
